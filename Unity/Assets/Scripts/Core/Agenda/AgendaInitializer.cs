...
namespace Wem.Agenda {
  ...
  public class AgendaInitializer {
    ...
    private void associateArea() {
      ...
      foreach (KeyValuePair<string, Area> area in this.map.Areas) {

        // For each activity allowed in the area, add a reference int the
        // activity.
        foreach (ActivityRef activity in area.Value.Activities) {
          this.container.GetActivity(activity).Areas.Add(area.Value);
        }

        if(area.Value.Link != null) {
        }
