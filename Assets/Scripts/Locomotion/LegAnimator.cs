...
{	
	...
	void Update() {
		...
		speed = velocityVClamped.magnitude;
		if (speed < 0.000001f && allParked)
		{
			
			return; // ASN: hack no work to do, just stand there
		}
		...
		for (int g=0; g<legC.motionGroups.Length; g++) {
			...
			if (group.weight > 0) {
				...
				for (int m=0; m<group.motionStates.Length; m++) {
					...
					//UnityEngine.Debug.Log("group "+ m+" " + weight+" "+group.controller.name+" "+group.relativeWeightsBlended[m]+" "+group.weight);
					// ASN HACK: Set blend mode to additive tso overlay motions work?
					group.controller.blendMode = AnimationBlendMode.Additive;
		...
		if (newWeights || updateStates) {
			...
			for (int m=0; m<legC.motions.Length; m++) {
				...
				//UnityEngine.Debug.Log("test: "+motionStates[m].weight+" "+motionStates[m].name);
			...
			if (summedCycleMotionWeight>0) {
				...
				for (int m=0; m<legC.cycleMotions.Length; m++) {
					...
					//UnityEngine.Debug.Log("cycle weights "+cycleMotionWeights[m]+" "+motion.name);
					if (weight>0) {
						for (int leg=0; leg<legs.Length; leg++) {
